...
-- Real-time Book Availability Trigger
...
BEGIN
  ...
  IF book_isbn IS NOT NULL THEN
    UPDATE books b
    SET availability = CASE 
      WHEN (
        SELECT COUNT(*) FROM transactions t 
        WHERE t.isbn = b.isbn AND t.type IN ('Borrow', 'Reservation') AND t.status = 'Approved'
      ) >= b.copies THEN 'Unavailable'
      ELSE 'Available'
    END
...
DROP TRIGGER IF EXISTS trg_update_book_availability ON transactions;
