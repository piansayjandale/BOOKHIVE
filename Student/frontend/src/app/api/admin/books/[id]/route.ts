...
export async function PATCH(request: Request, context: RouteContext<"/api/admin/books/[id]">) {
  ...
  try {
    ...
    if (rawPayload.archive === true || rawPayload.archived === true) {
      const archived = await adminRepository.archiveBook(id);
      if (!archived) {
    ...
    const book = await adminRepository.updateBook(id, payload, session);
    if (!book) {
      return NextResponse.json({ message: "Book not found." }, { status: 404 });
    }
...
export async function DELETE(_request: Request, context: RouteContext<"/api/admin/books/[id]">) {
  const session = await getSession();
  const { id } = await context.params;
  const deleted = await adminRepository.deleteBook(id, session);
